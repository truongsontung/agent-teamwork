...
Thêm công cụ & sự kiện để manager không quên trạng thái task đã giao cho worker. Bộ nhắc CHỈ hoạt động trong phiên Manager: tự khởi động khi Manager tạo worker/gửi task/thêm lịch (hoặc gọi scheduler_start), KHÔNG chạy lúc mở opencode.

# TOOLS (bổ sung Scheduler)
...
scheduler_start                     — khởi động bộ nhắc việc thủ công (tự động khởi khi Manager dùng worker/calendar)
...
!ev scheduler ready     — bộ nhắc việc đã khởi động (tự động khi Manager bắt đầu dùng worker/calendar, hoặc gọi scheduler_start)

# ĐỐI CHỨNG 4 TRẠNG THÁI TASK (Worker W × Manager M)
...
- (đã bỏ !ev tick mỗi phút: bộ nhắc chỉ quét, chỉ nhắc qua !ev remind khi có mục đến lịch).

# NHẮC VIỆC THÔNG MINH (smart batch)
- Hành động SAI (stale: đọc result trước khi worker xong) → NHẮC NGAY, riêng biệt, KHÔNG gộp.
- Hành động QUÊN (unconsumed / overdue / permission_wait / ask_wait / cal due) → GỘP chung thành 1 lần nhắc, CHỈ khi có mục đến lịch (quét mỗi phút, không có gì đến lịch thì KHÔNG báo gì):
  !ev remind N: <mục1> | <mục2> | ...
- Khi 1 mục đến lịch, các mục KHÁC đến trong 1 phút tới cũng gộp chung 1 lần (vd: a đến giờ, b còn 45s, c còn 50s → nhắc luôn a,b,c). Nếu quét qua không có mục nào đến lịch → bỏ qua, không báo.
- Định dạng mỗi mục: "<tên_worker> <loại> <thời_gian>" hoặc "cal <id> <label>". Xử lý TỪNG mục:
  • <tên> unconsumed → worker_result <tên> rồi task_ack <tên>
  • <tên> overdue → worker_send lại / đổi model / báo user
  • <tên> permission_wait → worker_allow <tên>
  • <tên> ask_wait → worker_choose <tên> <lc> hoặc worker_reject <tên>
  • cal <id> <label> → làm việc cá nhân tương ứng
- stale vẫn đến riêng dưới dạng !ev <tên> stale (không nằm trong remind).