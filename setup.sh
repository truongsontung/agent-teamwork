#!/bin/bash
# Agent Teamwork — setup Manager + Daemon (token-optimized)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="${PROJECT_DIR:-$PWD}"
export AGENT_TEAMWORK_HOME="$SCRIPT_DIR" PROJECT_DIR
...
(
    # Start worker bot (logs to file only)
    ...
    while true; do
        ...
        # Auto-allow permission (Enter on dialog)
...
echo "ready manager=$mgr_tool model=$mgr_model session=$SESSION pid=$DAEMON_PID"
