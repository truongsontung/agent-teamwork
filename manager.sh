...
create() {
    ...
    tmux send-keys -t "$SESSION:$name" "$DEFAULT_TOOL --model $model --agent worker --print-logs 2>/tmp/worker-$name.log" Enter
    echo "✓ $name created ($model, agent: worker, log: /tmp/worker-$name.log)"
}
